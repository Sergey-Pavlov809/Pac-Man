...
export class ControllerPointer extends ControllerBase {
  ...
  registerEvents(): void {
    document.addEventListener(this.startEventName, this.startControlByEvent, {
      passive: false,
    })
    document.addEventListener(this.stopEventName, this.stopControlByEvent, {
      passive: false,
    })
  }
  ...
  getPointerBinding(target: EventTarget): Binding | null | undefined {
    if (
